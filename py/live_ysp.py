...
"""
央视频直播代理插件（支持直播+7天回看）
- 完全模仿 4GTV.py 架构，网络请求由 Spider 处理
- 优先使用 TVBox 自带的 self.fetch / self.post
- 仅当 TVBox 无 fetch 时降级 urllib
- 保留完整加密和频道数据
"""
...
class CKeyManager:
    ...
    def build_playback_params(self, cnlid, livepid, defn, playback_timestamp):
        params = self.build_live_params(cnlid, livepid, defn)
...
class Spider(BaseSpider):
    ...
    def _http_post_json(self, url, payload, headers=None, timeout=15):
        """发送 JSON POST，优先使用 self.post，降级 urllib"""
        if headers is None:
            headers = {}
        ...
        try:
            if hasattr(self, 'post'):
                resp = self.post(url, data=data, headers=headers, timeout=timeout)
                if hasattr(resp, 'text'):
                    return resp.text
                elif hasattr(resp, 'content'):
                    return resp.content.decode('utf-8', errors='ignore')
        ...
        try:
            ...
            log(f"urllib POST 失败: {e}", 'ERROR')
            return None

    # ---------- 央视频专用请求 ----------
    def _request_cctv_live(self, params):
        """向 bkliveinfo 发送 GET 请求"""
        ...
        text = self._http_get(full_url, headers=headers, timeout=15)
        if text is None:
            return None
        try:
            ...
            if fun == 'cctv':
                ...
                if playseek:
                    ...
                    try:
                        ...
                        manager = CKeyManager()
                        req_params = manager.build_playback_params(
                            ch['cnlid'], ch['livepid'], ch['defn'], playback_timestamp
                        )
                        playurl = self._request_cctv_live(req_params)
                        if not playurl:
                    ...
                    if not m3u8_content:
                        ...
                        manager = CKeyManager()
                        req_params = manager.build_live_params(ch['cnlid'], ch['livepid'], ch['defn'])
                        playurl2 = self._request_cctv_live(req_params)
                        if not playurl2:
