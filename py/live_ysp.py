...
- 日志级别设为 INFO，减少输出
- 保留关键错误和请求日志
- 可通过环境变量 CCTV_DEBUG=1 开启 DEBUG 日志
"""
...
class CKeyManager:
    ...
    def make_playback_request(self, cnlid, livepid, defn, playback_timestamp):
        ...
        # 如果回看失败，尝试不带playbacktime参数（降级到直播）
        if not result.get('success'):
            logger.warning("回看请求失败，尝试降级到直播")
    ...
    def send_http_request(self, params):
        ...
        try:
            ...
            logger.debug(f"请求参数（完整）: {json.dumps(params, ensure_ascii=False, indent=2)}")
            ...
            if resp.status_code == 200:
                ...
                if len(json_str) > 2000:
                    logger.debug(f"响应 JSON（前2000字符）: {json_str[:2000]} ... (总长度 {len(json_str)})")
                else:
                    logger.debug(f"响应 JSON: {json_str}")
                ...
                logger.error(f"HTTP 错误码: {resp.status_code}, 内容: {resp.text[:500]}")
...
class Spider(BaseSpider):
    ...
    def _fetch_and_fix_m3u8(self, play_url):
        try:
            ...
            logger.debug(f"M3U8 响应状态码: {resp.status_code}")

            if resp.status_code != 200:
                logger.error(f"M3U8 获取失败 HTTP {resp.status_code}")
                logger.error(f"响应内容前200字符: {resp.text[:200] if resp.text else '(空)'}")
                return None

            content = resp.text
            logger.debug(f"原始 M3U8 总长度: {len(content)} 字符")
            lines = content.splitlines()
            logger.debug(f"原始 M3U8 总行数: {len(lines)}")
            if len(lines) > 30:
                logger.debug("原始 M3U8 前30行:\n" + '\n'.join(lines[:30]))
                logger.debug("原始 M3U8 后10行:\n" + '\n'.join(lines[-10:]))
            else:
                logger.debug("原始 M3U8 全部内容:\n" + content)
            ...
            logger.debug(f"基础 URL 用于补全 TS: {base}")
            ...
            for line in lines:
                ...
                if stripped and not stripped.startswith('#') and '.ts' in stripped:
                    ...
                    if not stripped.startswith(('http://', 'https://')):
                        ...
                        if ts_count <= 5:
                            ts_examples.append(f"{stripped} -> {new_url}")
                    else:
                        fixed_lines.append(line)
                        if ts_count <= 5:
                            ts_examples.append(f"(绝对路径) {stripped}")
                else:
                    fixed_lines.append(line)

            logger.info(f"共处理 {ts_count} 个 TS 链接")
            if ts_examples:
                logger.debug("TS 替换示例:\n" + '\n'.join(ts_examples))

            result = '\n'.join(fixed_lines)
            logger.debug(f"补全后 M3U8 总长度: {len(result)} 字符")
            result_lines = result.splitlines()
            if len(result_lines) > 20:
                logger.debug("补全后 M3U8 前20行:\n" + '\n'.join(result_lines[:20]))
                logger.debug("补全后 M3U8 后10行:\n" + '\n'.join(result_lines[-10:]))
            else:
                logger.debug("补全后 M3U8 全部内容:\n" + result)
            return result
        except Exception as e:
    ...
    def localProxy(self, params):
        ...
        try:
            fun = params.get('fun')
            logger.info(f"请求的 fun 参数: {fun}")

            if fun == 'cctv':
                channel_id = params.get('id')
                if not channel_id:
                    logger.warning("缺少频道ID")
                ...
                if playseek:
                    ...
                    try:
                        ...
                        if len(parts) == 2:
                            ...
                            logger.info(f"回看时间戳: {playback_timestamp}")
                            ...
                            if not m3u8_content:
                                logger.warning("回看获取 M3U8 失败，尝试直播")
                                # 降级到直播
                            ...
                            logger.warning(f"playseek格式错误: {playseek}")
                    ...
                    # 正常直播流程（完全保持原样）
                    logger.info(f"直播请求: {channel_id} ({ch['name']}), cnlid={ch['cnlid']}, livepid={ch['livepid']}, defn={ch['defn']}")

                    playurl, valid = self._get_cached_playurl(channel_id)
                    if not valid:
                        logger.info("缓存失效或不存在，开始请求 playurl")
                        manager = CKeyManager()
                        playurl = manager.get_play_url(ch['cnlid'], ch['livepid'], ch['defn'])
                        if not playurl:
                            logger.error("获取 playurl 失败（首次）")
    ...
    def _get_live_fallback(self, ch):
        ...
        if not valid:
            ...
            playurl = manager.get_play_url(ch['cnlid'], ch['livepid'], ch['defn'])
            if not playurl:
                return self._error_response("降级获取直播地址失败")
            self._set_cached_playurl(channel_id, playurl)
        
        m3u8_content = self._fetch_and_fix_m3u8(playurl)
        if not m3u8_content:
            return self._error_response("降级获取M3U8内容失败")
...
if __name__ == '__main__':
    ...
    playseek = f"{yesterday}-{yesterday}"
    result = spider.localProxy({'fun': 'cctv', 'id': 'cctv1', 'playseek': playseek})
    if result[0] == 200:
        print("✅ 回看获取成功，M3U8 内容长度:", len(result[2]))
    else:
        print("❌ 回看失败:", result[2])
