...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        self._ytcfg_cache = None   # 新增：缓存 ytcfg
    ...
    # ---------- 新增：获取 ytcfg（缓存） ----------
    def _get_ytcfg(self):
        if self._ytcfg_cache is not None:
            return self._ytcfg_cache
        try:
            r = self.session.get('https://www.youtube.com/', timeout=10)
            html = r.text
            m = re.search(r'ytcfg\.set\s*\(\s*({.+?})\s*\)\s*;', html, re.S)
            if m:
                self._ytcfg_cache = json.loads(m.group(1))
            else:
                self._ytcfg_cache = {}
        except Exception:
            self._ytcfg_cache = {}
    ...
    # ---------- 新增：调用 browse 接口 ----------
    def _call_browse_api(self, browse_id, token='', context_override=None):
        ytcfg = self._get_ytcfg()
        api_key = ytcfg.get('INNERTUBE_API_KEY')
        if not api_key:
            api_key = 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'  # 备用
        context = ytcfg.get('INNERTUBE_CONTEXT') or {
            'client': {'clientName': 'WEB', 'clientVersion': '2.20240310.01.00', 'hl': 'zh-CN', 'gl': 'US'}
        }
        if context_override:
    ...
    def _parse_playlist_renderer(self, renderer):
        try:
            ...
            title = title_obj.get('simpleText') or ''.join([x.get('text', '') for x in title_obj.get('runs', [])]) or '播放列表'
            count_text = renderer.get('videoCountShortText') or renderer.get('videoCountText') or {}
            count = count_text.get('simpleText', '')
            thumbs = renderer.get('thumbnails') or []
            pic_url = ''
            if thumbs and thumbs[0].get('thumbnails'):
                pic_url = thumbs[0]['thumbnails'][-1].get('url', '')
            return {
                'vod_id': 'PL' + plid,   # 前缀标识
                'vod_name': html.unescape(title),
                'vod_pic': pic_url,
                'vod_remarks': count if count else '播放列表'
            }
        except Exception:
            return None

    # ---------- 新增：从 browse 响应中提取视频和播放列表 ----------
    def _extract_videos_and_playlists_from_browse(self, data, limit=30):
        videos = []
        seen = set()
        def scan(obj):
            if len(videos) >= limit:
                ...
                for key in ('videoRenderer', 'compactVideoRenderer', 'gridVideoRenderer'):
                    if key in obj:
                        ...
                        if item and item['vod_id'] not in seen:
                            ...
                            videos.append(item)
                # 播放列表
                for key in ('playlistRenderer', 'gridPlaylistRenderer', 'compactPlaylistRenderer'):
                    if key in obj:
                        item = self._parse_playlist_renderer(obj[key])
                        if item and item['vod_id'] not in seen:
                            seen.add(item['vod_id'])
                            videos.append(item)
    ...
    # ---------- 新增：频道浏览 ----------
    def _browse_channel(self, channel_id, page=1):
        data = self._call_browse_api(channel_id)
        return self._extract_videos_and_playlists_from_browse(data, limit=30)

    # ---------- 新增：播放列表浏览 ----------
    def _browse_playlist(self, playlist_id, page=1):
        browse_id = 'VL' + playlist_id
        data = self._call_browse_api(browse_id)
        return self._extract_videos_and_playlists_from_browse(data, limit=30)

    # ---------- 新增：推荐/热门 browse ----------
    def _browse_fe(self, fe_id, page=1):
        data = self._call_browse_api(fe_id)
        videos = self._extract_videos_and_playlists_from_browse(data, limit=30)
        token = self._extract_continuation_token(data)
        return videos, bool(token)

    # ---------- 新增：播放列表详情（多集拉取） ----------
    def _detail_playlist(self, playlist_id):
        all_videos = []
        token = ''
        for _ in range(5):   # 最多拉5页
            data = self._call_browse_api('VL' + playlist_id, token=token)
            items = self._extract_videos_and_playlists_from_browse(data, limit=100)  # 不严格限制
            # 只保留视频（排除播放列表条目）
            video_items = [x for x in items if not x['vod_id'].startswith('PL')]
        ...
        if not all_videos:
            return {'list': [{'vod_id': playlist_id, 'vod_name': '播放列表为空', 'vod_play_from': '', 'vod_play_url': ''}]}

        # 构造多集线路
        episodes = []
        for idx, v in enumerate(all_videos, 1):
            ep_name = f"{idx:02d} {v['vod_name']}"
            ep_url = f"{ep_name}${v['vod_id']}@best"
            episodes.append(ep_url)

        play_from = '播放列表'
        play_url = '#'.join(episodes)
        pic = all_videos[0].get('vod_pic', '') if all_videos else ''
        title = f"播放列表 {playlist_id}"

        vod = {
            'vod_id': 'PL' + playlist_id,
            'vod_name': title,
            'vod_pic': pic,
            'vod_remarks': f"共{len(all_videos)}集",
    ...
    def categoryContent(self, cid, page, filter, ext):
        ...
        # 判断 cid 类型
        if cid.startswith('UC'):          # 频道
            videos = self._browse_channel(cid, page)
            has_more = False
            total = len(videos)
        elif cid.startswith('PL'):        # 播放列表（直接传入 PLxxx）
            videos = self._browse_playlist(cid[2:], page)
            has_more = False
            total = len(videos)
        elif cid.startswith('FE'):        # 推荐/热门
            videos, has_more = self._browse_fe(cid, page)
            total = len(videos)
        else:
            # 原有搜索逻辑
    ...
    def detailContent(self, did):
        video_id = did[0]
        # 判断是否为播放列表
        if video_id.startswith('PL'):
            plid = video_id[2:]
            return self._detail_playlist(plid)
        else:
            # 原有单视频逻辑（保留）
            return self._detail_video(video_id)

    # 将原有单视频逻辑抽取为 _detail_video
    def _detail_video(self, video_id):
        # 检测是否为直播
        try:
        ...
        return {'list': [vod]}
    # ---------- 原有方法保持不变（从 homeContent 到 destroy） ----------
