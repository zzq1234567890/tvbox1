...
"""
央视频直播代理（精简稳定版）
- 仅依赖 Python 标准库 + TVBox 自带的 fetch（优先）
- 所有网络请求均使用 urllib（若 fetch 不可用）
- 详细日志写入 /sdcard/Download/cctv.log
- 异常捕获确保即使出错也返回错误 M3U，避免空白列表
"""
...
class Spider(BaseSpider):
    ...
    # ---------- 通用 HTTP 请求（优先 self.fetch，降级 urllib） ----------
